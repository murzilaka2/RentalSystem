...
{
    public class UpdateRentalModel : PageModel
    {
        ...
        public UpdateRentalModel(IRental rentals, IWebHostEnvironment environment)
        {
            _rentals = rentals;
        }
        ...
        public async Task<IActionResult> OnPostUpdateRentalAsync()
        {
            ...
            if (action.Equals("CloseLease"))
            {
                await _rentals.CloseRentalAsync(RentalModel.Id);
                TempData["SuccessfullyChanged"] = true;
                return Redirect("/adminDashboard/updateRental?id=" + RentalModel.Id);
            }
            else if (action.Equals("CancelLease"))
            {
